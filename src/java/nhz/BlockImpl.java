...
final class BlockImpl implements Block {
    ...
    BlockImpl(int version, int timestamp, Long previousBlockId, long totalAmountNQT, long totalFeeNQT, int payloadLength, byte[] payloadHash,
        ...
        if (transactions.size() > Constants.MAX_NUMBER_OF_TRANSACTIONS) {
            throw new NhzException.ValidationException("attempted to create a block with " + transactions.size() + " transactions");
        }

        if (payloadLength > Constants.MAX_NUMBER_OF_TRANSACTIONS * (version < 3 ? 128 : 160)|| payloadLength < 0) {
            throw new NhzException.ValidationException("attempted to create a block with payloadLength " + payloadLength);
        }
        ...
        for (Transaction transaction : this.blockTransactions) {
            if (transaction.getId() < previousId) {
                throw new NhzException.ValidationException("Block transactions are not sorted!");
            }
    ...
    public JSONObject getJSONObject() {
        ...
        if (version < 3) {
            json.put("totalAmount", totalAmountNQT / Constants.ONE_NHZ);
            json.put("totalFee", totalFeeNQT / Constants.ONE_NHZ);
        }
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
        ...
        try {
            ...
            BigInteger target = BigInteger.valueOf(Nhz.getBlockchain().getLastBlock().getBaseTarget())
