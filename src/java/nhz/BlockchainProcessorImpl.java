        ...
        try {
            ...
            for (int i = 0; i < Genesis.GENESIS_RECIPIENTS.length; i++) {
                TransactionImpl transaction = new TransactionImpl(TransactionType.Payment.ORDINARY, 0, (short) 0, Genesis.CREATOR_PUBLIC_KEY,
                        Genesis.GENESIS_RECIPIENTS[i], Genesis.GENESIS_AMOUNTS[i] * Constants.ONE_NHZ, 0, (String)null, Genesis.GENESIS_SIGNATURES[i]);
    ...
    private BlockImpl parseBlock(JSONObject blockData) throws NhzException.ValidationException {
        try {
            ...
            if (blockData.get("totalAmountNQT") != null) {
                ...
                totalAmountNQT = Convert.safeMultiply(((Long) blockData.get("totalAmount")), Constants.ONE_NHZ);
                totalFeeNQT = Convert.safeMultiply(((Long) blockData.get("totalFee")), Constants.ONE_NHZ);
            }
            ...
            for (Object transactionData : transactionsData) {
                ...
                if (blockTransactions.put(transaction.getId(), transaction) != null) {
                    throw new NhzException.ValidationException("Block contains duplicate transactions: " + transaction.getStringId());
                }
            }
                ...
                try {
                    while (rs.next()) {
                        ...
                        if (! currentBlock.getId().equals(currentBlockId)) {
                            throw new NhzException.ValidationException("Database blocks in the wrong order!");
                        }
                        if (validateAtScan && ! currentBlockId.equals(Genesis.GENESIS_BLOCK_ID)) {
                            if (!currentBlock.verifyBlockSignature() || !currentBlock.verifyGenerationSignature()) {
                                throw new NhzException.ValidationException("Invalid block signature");
                            }
                            if (! verifyVersion(currentBlock, blockchain.getHeight())) {
                                throw new NhzException.ValidationException("Invalid block version");
                            }
                            for (TransactionImpl transaction : currentBlock.getTransactions()) {
                                if (!transaction.verify()) {
                                    throw new NhzException.ValidationException("Invalid transaction signature");
                                }
                                transaction.validateAttachment();
                            }
                        }
