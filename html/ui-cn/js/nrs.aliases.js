var NRS = (function(NRS, $, undefined) {
	...
	NRS.forms.setAliasType = function(type, uri) {
		...
		if (type == "uri") {
			...
			$("#register_alias_help").html("别名将和您输入的帐号绑定，且可用收接收NHZ，或者消息..").show();
		} else {
	...
	NRS.forms.assignAliasComplete = function(response, data) {
		...
		if (NRS.currentPage == "aliases") {
			...
			if ($row.length) {
				...
				var rowToAdd = "<tr class='tentative' data-alias='" + data.alias.toLowerCase().escapeHTML() + "'><td class='alias'>" + data.alias.escapeHTML() + " -  <strong>处理中</strong></td><td class='uri'>" + (data.uri && data.uri.indexOf("http") === 0 ? "<a href='" + String(data.uri).escapeHTML() + "' target='_blank'>" + data.uri.escapeHTML() + "</a>" : String(data.uri).escapeHTML()) + "</td><td>编辑</td></tr>";
