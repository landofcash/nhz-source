	...
	NRS.getAccountError = function(accountId, callback) {
		NRS.sendRequest("getAccount", {
			...
			if (response.publicKey) {
				callback({
					"type": "info",
					"message": "The recipient account has a public key and a balance of " + NRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + "NHZ.",
					"account": response
				});
			} else {
				if (response.errorCode) {
					if (response.errorCode == 4) {
						callback({
							"type": "danger",
							"message": "The recipient account is malformed, please adjust." + (!/^(NHZ\-)/i.test(accountId) ? " If you want to type an alias, prepend it with the @ character." : ""),
							"account": null
						});
					...
					callback({
						"type": "warning",
						"message": "The recipient account does not have a public key, meaning it has never had an outgoing transaction. The account has a balance of " + NRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " NHZ. Please double check your recipient address before submitting.",
						"account": response
					});
				}
			}
