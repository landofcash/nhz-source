...
function addAlias(alias, type) {
	var template = Handlebars.templates['alias'];
	...
	if (a.uri.substr(0, 5) == "acct:" && a.uri.substr(-4) == "@nhz") {
		a.aliasType       = Language.get('labels.nhzAccount');
		a.aliasTypeNumber = 1;

		a.filters += " nhz-account";
	}
...
function formatAliasUri(uri) {
	...
	if (uri.substr(0, 5) == "acct:" && uri.substr(-4) == "@nhz") {
	...
	if ($(selector).val() != "") {
		...
		$('#my-aliases tr').each(function(){
			if (alias.toLowerCase() == $(this).children('.alias').attr('data-alias').toLowerCase()) {
				...
				if (uri.substr(0, 5) == "acct:" && uri.substr(-4) == "@nhz") {
					$('#register-alias-type').val('Nhz Account');
