...
function addBlock(block, type) {
	var template = Handlebars.templates['block'];
	...
	//set transaction URL
	b.id = b.block;
	b.blockUrl = config.blockUrl.replace('[blockId]', b.height);

	//set timestamps
	b.timestamp          = new Date(Date.UTC(2014, 02, 22, 22, 22, 0, 0) + b.timestamp * 1000);
	...
	$("ul#recent-blocks li").slice(config.showMaxBlocks).remove();
...
function removeBlock(block, type) {
	updateTransactionConfirmations(-1);
