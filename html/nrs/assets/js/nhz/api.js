/*
|------------------------------------------------------------------------------
| Api.js: API JS interface for Nhz/Nxs client
|------------------------------------------------------------------------------
|
...
var Api = {
	...
	sendRequest: function(requestType, data, uiServer) {
		...
		if (uiServer !== true)
			url = (config.ssl ? "https://" : "http://") + "localhost:" + config.apiServerPort + "/" + url;
		else
			data.user = user;
	...
	processRequest: function(response, data) {
		...
		switch (data.requestType)
		{
			...
			case "getAliasURI":
				...
				//check Nhz (NRS) version against latest version
				if ($.inArray(data.alias, ['NRSversion', 'NRSbetaversion']) >= 0) {
					if (response.uri !== undefined) {
						var versionData  = response.uri.split(' ');
						nhzLatestVersion = versionData[0];
						nhzLatestHash    = versionData[1];

						$('tr.nhz td.latest-version')
							.text(nhzLatestVersion)
							.attr('title', nhzLatestHash);

						var comparison = versionCompare(nhzVersion, nhzLatestVersion);
						if (comparison < 0 || comparison > 0) {
							$('tr.nhz td.version .outdated').removeClass('hidden');
							$('tr.nhz td.update a').removeClass('hidden');
						} else {
							$('tr.nhz td.version .up-to-date').removeClass('hidden');
						}
					}
				}

				//adjust link URL for Nhz's "Update" button
				if ($.inArray(data.alias, ['NRSrelease', 'NRSbetarelease']) >= 0) {
					nhzLatestFileUrl = response.uri;
					$('tr.nhz td.actions a').attr('href', '#').removeAttr('target');
				}
			...
			case "getBlock":
				//check to see if Nhz is running on live network or test network
				if (!testNetChecked) {
					if (data.block == secondBlock && response.height !== 1) {
						config.testNet       = true;
						config.apiServerPort = 7775;
			...
			for (i = 0; i < responses.length; i++) {
				...
				switch (response.response)
				{
					...
					case "processInitialData":
						...
						nhzVersion = response.version;

						$('.nhz-version').text(response.version);
						$('.nxs-version').text(config.version);
						$('#footer .versions').fadeIn('slow');

						$('tr.nhz span.version').text(response.version);
