...
function addPeer(peer, type) {
	var template = Handlebars.templates['peer'];
...
function changePeer(peer, type) {
	try {
		...
		if($.trim($('tr.peer-' + peer.index + ' td.enabled-announced-address').html()) == '-') {
			removePeer(peer, type);
			row.remove();
		}
		...
		if (peer.downloaded) {
			...
			row.find('td.downloaded').effect('highlight')
				.text(bytesToSize(peer.downloaded))
				.attr('title', Language.get('labels.downloaded') + ': ' + peer.downloaded + ' B');
		}
		...
		if (peer.uploaded) {
			...
			row.find('td.uploaded').effect('highlight')
				.text(bytesToSize(peer.uploaded))
				.attr('title', Language.get('labels.uploaded')   + ': ' + peer.uploaded   + ' B');
		}
	} catch(e) {}
}
