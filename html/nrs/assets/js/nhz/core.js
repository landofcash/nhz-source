...
function initialize() {
	...
	//set default fee
	$('input.fee').val(config.defaultFee);
	...
	$('#account').click(function(){
		if (account !== false) {
			...
			$('#qr-code').html('').qrcode('nhzacct:' + account);
	...
	var loadNewData = setInterval("Api.sendUiRequest('getNewData')",config.dataRequestWaitTime);

	$("#send-amount").bind("keyup change", function(){
		amount = $("#send-amount").val();		
		if (amount > balance) {
			fee = Math.ceil(balance / 1000);
			amount = balance - fee;
		}
		fee = Math.ceil(amount / 1000);
		$("#send-fee").val(fee);
...
function initializeSecretPhraseFields() {
	$('.show-secret-phrase').click(function(){
		...
		if (! $(this).prop('checked')) {
			...
			$('#'+where+'-secret-phrase-visible').addClass('hidden');
			$('#'+where+'-secret-phrase').removeClass('hidden').focus();
		} else {
			...
			$('#'+where+'-secret-phrase').addClass('hidden');
			$('#'+where+'-secret-phrase-visible').removeClass('hidden').focus();
		}
	});
...
function clearRecipientData() {
	$("#send-recipient").val("");
	$("#send-amount").val("0.00000000");
	$("#send-fee").val("0.00000000");
}
...
function initializeModalAbout() {
	$('#update-nhz').click(function(){
		$('#nhz-hash-check-target').hide().removeClass('hidden').fadeIn();
